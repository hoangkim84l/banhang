...
define('RS_TP_TOOLS',			'6.0.7'); //holds the version of the tp-tools script, load only the latest!
...
function revslider_as_theme() {
	...
	if ( file_exists( $theme_core ) ) {
		...
		if  (function_exists("presscore_is_silence_enabled") && presscore_is_silence_enabled()){
			define('REVSLIDER_THEME_ACT', true);
			define('REVSLIDER_THEME_INIT', true);
			return;
		}
	}
	define('REVSLIDER_THEME_INIT', true);
}
add_action('after_setup_theme', 'revslider_as_theme');
